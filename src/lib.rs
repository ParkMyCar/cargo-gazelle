...
///
/// let deps = List::new("deps", vec![QuotedString::new("tokio")]);
/// assert_eq!(deps.to_bazel_definition(), "deps = [\"tokio\"],\n");
/// ```
#[derive(Debug)]
pub struct List<T> {
    items: Vec<T>,
