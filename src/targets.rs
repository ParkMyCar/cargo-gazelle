...
impl RustTest {
    fn common(
        ...
        Ok(RustTest {
            ...
            compile_data: Field::new("compile_data", List::empty()),
            deps: Field::new("deps", deps),
            proc_macro_deps: Field::new("proc_macro_deps", proc_macro_deps),
    ...
    pub fn iter(
        ...
        self.package
            ...
            .map(|(path, meta)| {
                ...
                format!("//{}:{}", path.to_string(), crate_name)
            })
    }
}
...
pub fn crate_features<'a>(
    ...
    // Collect into a HashSet to de-dupe.
    let features: HashSet<_> = features
        .into_iter()
...
mod tests {
    ...
    // #[test]
    // fn smoketest_library() {
    //     let lib = RustLibrary {
    //         name: "my_crate".to_string(),
    //         features: List::new("crate_features", vec![QuotedString::new("json")]),
    //         compile_data: List::empty("compile_data"),
    //         aliases: List::empty("aliases")
    //             .with_default_set("aliases(normal = True, proc_macro = True)"),
    //         deps: List::new(
    //             "deps",
    //             vec![
    //                 QuotedString::new("//src/foo:lib"),
    //                 QuotedString::new("//src/bar:lib"),
    //             ],
    //         ),
    //         proc_macro_deps: List::empty("proc_macro_deps")
    //             .with_default_set("all_crate_deps(proc_macro = True)"),
    //     };

    //     insta::assert_snapshot!(lib.to_bazel_definition(), @r###"
    //     rust_library(
    //     	name = "my_crate",
    //     	srcs = glob(["src/**/*.rs"]),
    //     	crate_features = ["json"],
    ...
    // #[test]
    // fn smoketest_test() {
    //     let unit_test = RustTest {
    //         name: "my_crate_unit_test".to_string(),
    //         kind: RustTestKind::Library {
    //             crate_path: ":my_crate".to_string(),
    //         },
    //         compile_data: vec![],
    //         deps: vec![],
    //         proc_macro_deps: vec!["//src/my_test_macro:test_macro".to_string()],
    //         size: RustTestSize::Small,
    //     };
    //     insta::assert_snapshot!(unit_test.to_bazel_definition(), @r###"
    //     rust_test(
    //     	name = "my_crate_unit_test"
    //     	crate = :my_crate,
    //     	aliases = aliases(
    //     		normal = True,
    //     		normal_dev = True,
    ...
    //     let integration_test = RustTest {
    //         name: "cool_test".to_string(),
    //         kind: RustTestKind::Integration {
    //             srcs: vec!["foo_1".to_string(), "foo_2".to_string()],
    //         },
    //         compile_data: vec![],
    //         deps: vec![":my_crate".to_string()],
    //         proc_macro_deps: vec!["//src/my_test_macro:test_macro".to_string()],
    //         size: RustTestSize::Small,
    //     };
    //     insta::assert_snapshot!(integration_test.to_bazel_definition(), @r###"
    //     rust_test(
    //     	name = "cool_test"
    //     	srcs = [
    //     		"foo_1",
